...
if ($view['link']) {
	...
	for ($i=1; $i<=count($view['link']); $i++) {
		if ($view['link'][$i]) {
			...
			$attach_list .= '관련링크 <span class="label label-warning pull-right view-cnt">'.number_format($view['link_hit'][$i]).'</span>';
...
<div class="view-wrap<?php echo (G5_IS_MOBILE) ? ' view-mobile font-14' : '';?>">
	...
	<div class="panel view-head<?php echo ($attach_list) ? '' : ' no-attach';?>">
		<div class="panel-heading">
			<div class="font-1-5r text-muted">
	   ...
	   <?php
			if($attach_list) {
				echo '<div class="list-group font-12 link">'.$attach_list.'</div>'.PHP_EOL;
			}
		?>
	</div>
			...
			<div class="form-group">
				...
				<?php if ($reply_href) { ?>
					<a href="<?php echo $reply_href ?>" class="btn btn-black btn-sm">
						<i class="fa fa-comments"></i><span class="hidden-xs"> 답글</span>
					</a>
				<?php } ?>
				<?php if ($write_href) { ?>
