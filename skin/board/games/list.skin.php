...
<section class="board-list<?php echo (G5_IS_MOBILE) ? ' font-14' : '';?>">
	...
	<?php if($member['admin']) { ?>
		...
		<?php if ($write_href) { ?><a href="<?php echo $write_href ?>"> <i class="fa fa-pencil"> </i></a><?php } ?>
		</div>
	<?php } ?>
	...
	<div class="list-wrap">
		<form name="fboardlist" id="fboardlist" action="./board_list_update.php" onsubmit="return fboardlist_submit(this);" method="post" role="form" class="form">
			...
			<div class="scrollTop list-container">
			<?php 
				$k = 0;
	...
	function callContent(a,b){
		...
		$.ajax({
			...
			success: function(html){
				...
				setTimeout(function() { 
					if(b=='append'){
						$(".list-row").find('img').append(thtml[0]);
					}else{
						$(".list-row").find('img').prepend(thtml[0]);
					}
					$(".moreBar").slideUp();
...
//	let imgCnt = $(".list-row").length;
//
//	for (var k=0; k < imgCnt; k++) {
//		let list_item = $(".list-item"+k);		
//		let img = $(".img-wrap");
//		let randW = Math.floor(Math.random() * 200);
//		let randH = Math.floor(Math.random() * 300);
//
//		for(var i = 0; i < imgCnt; i++){
//			var idx = Math.floor(Math.random() * imgCnt -1) + 1;
//			var idx2 = Math.floor(Math.random() * imgCnt -1) + 1;
//			list_row.eq(idx).before(list_row.eq(idx2));
//		};
//		list_row.height(randH + randH + 'px');
//		list_item.height(randH + randH + 'px');
//		img.width(200 + 'px');
//		img.height(randH + 'px');
//	}
</script>
