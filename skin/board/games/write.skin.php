...
<?php } ?>
	.write-wrap .f-ex{ margin:10px;font-size:2rem; }
	.item-label label { font-weight: normal !important; }
	.form-group .col-sm-6 { width:25%; }
	.form-group .col-sm-10 { width:100%; }
	.form-group .col-sm-10 .btn-color, .form-group .col-sm-10 .btn-black { font-size: 3rem;line-height: 0;border-radius: 4rem; }
</style>
...
<div id="bo_w" class="write-wrap<?php echo (G5_IS_MOBILE) ? ' font-14' : '';?>">
	...
	<?php if ($is_name && $is_password) { ?>
		<div class="form-group has-feedback">
			<div class="col-sm-3">
				<input type="text" name="wr_name" id="wr_name" value="<?php echo $name ?>" required class="form-control input-sm required" size="10" maxlength="20" placeholder="이름">
			</div>
			<div class="col-sm-3">
				<input type="password" name="wr_password" id="wr_password" <?php echo $password_required ?> class="form-control input-sm required" maxlength="20" placeholder="비밀번호">
			</div>
		</div>
	<?php } ?>
	<!-- 이메일 && 홈페이지 -->
		...
		<div class="form-group">
			<div class="col-sm-3">
				<input type="text" name="wr_email" id="wr_email" value="<?php echo $email ?>" class="form-control input-sm email" size="10" maxlength="100" placeholder="Email">
			</div>
			<div class="col-sm-3">
				<input type="text" name="wr_homepage" id="wr_homepage" value="<?php echo $homepage ?>" class="form-control input-sm" size="10" placeholder="홈페이지">
			</div>
		</div>
	<?php } ?>
	<!-- 분류 -->
	...
	<div class="form-group">
		<div class="col-sm-10">
			<div class="input-group">
				<input type="text" name="wr_subject" value="<?php echo $subject ?>" id="wr_subject" class="form-control input-sm required" size="50" maxlength="255" placeholder="제목" required >
				<span class="input-group-btn">
					<a href="<?php echo G5_BBS_URL;?>/helper.php" target="_blank" class="btn btn-sm hidden-xs win_scrap">안내</a>
					<a href="<?php echo G5_BBS_URL;?>/helper.php?act=map" target="_blank" class="btn btn-sm hidden-xs win_scrap">지도</a>
					<?php if ($is_member) { // 임시 저장된 글 기능 ?>
						<button type="button" id="btn_autosave" data-toggle="modal" data-target="#autosaveModal" class="btn btn-sm">저장 (<span id="autosave_count"><?php echo $autosave_count; ?></span>)</button>
					<?php } ?>
				</span>
			</div>
		</div>
	...
	<!-- 태그 -->
	<?php if($is_admin || ($boset['tag'] && $member['mb_level'] >= $boset['tag'])) { ?>
		<div class="form-group">
			<div class="col-sm-10">
				<input type="text" name="as_tag" id="as_tag" value="<?php echo $write['as_tag']; ?>" class="form-control input-sm" size="50" placeholder="태그">
			</div>
		</div>
	<?php } ?>
	<!-- 링크 -->
	<?php for ($i=2; $is_link && $i<=G5_LINK_COUNT; $i++) { ?>
		<div class="form-group">
			<div class="col-sm-10">
				<input type="text" name="wr_link<?php echo $i ?>" value="<?php if($w=="u"){ echo $write['wr_link'.$i]; } ?>" id="wr_link<?php echo $i ?>" class="form-control input-sm" size="50" placeholder="링크">
			</div>
		</div>
	<?php } ?>	
	<?php if ($is_file) { ?>
