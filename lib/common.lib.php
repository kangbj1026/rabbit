...
// html/page/lists 페이지 리스트 뽑기
function page_list($titleName) {
$result = sql_query("select * from g5_apms_page where as_html = '1' order by gr_id asc, id ");
	for($z=0;$row=sql_fetch_array($result);$z++){
		$title[] = $row['bo_subject'];
		$p_list[] = $row['html_id'];
	}
	for( $i = 0; $i < count($p_list); $i++ )
	{
		$a = "<a name='bo_subject[$i]' href=/bbs/".G5_PAGE_DIR.".php?hid=$p_list[$i]>".$title[$i]."</a>";
		echo "<li>".$a."</li>";
	}
}
function tailLink() {
	$t_link = ["naver","netflix","rabbit.dothome"];
	echo "<div class='quickmenu'><ul>";
	for($l = 0;$l < count($t_link);$l++)
	{
		if($t_link[$l] == "rabbit.dothome"){
			$t_c = "<li><a href='http://".$t_link[$l].".co.kr' target='_blank'>".$t_link[$l]."</a></li>";
			echo $t_c;
		} else if($t_link[$l] == "naver" || "netflix") {
			$t_c = "<li><a href='https://www.".$t_link[$l].".com' target='_blank'>".$t_link[$l]."</a></li>";
			echo $t_c;
		}
	}
	echo "</ul></div>";
